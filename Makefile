...
bootstrap: ## check tools, copy .env, install deps, build shared types
	...
	pnpm turbo build --filter=@relay-ai/shared
...
dev: up ## inner loop: core + mockllm container + watch server/console
	...
	$(LOADENV) pnpm turbo dev --filter=!@relay-ai/mockllm
...
migrate: ## apply SQL migrations (advisory-locked)          [sprint Day 2]
	$(LOADENV) pnpm --filter @relay-ai/server exec tsx src/cli/index.ts migrate

seed-auth: ## idempotent Logto bootstrap                     [sprint Day 5]
	$(LOADENV) pnpm --filter @relay-ai/server exec tsx src/cli/index.ts seed-auth

seed-demo: ## demo org+app+key+route -> prints working curl  [sprint Day 5]
	$(LOADENV) pnpm --filter @relay-ai/server exec tsx src/cli/index.ts seed-demo

generate: ## dump OpenAPI spec + regen the console's typed client + Postman collection  [sprint Day 2+]
	pnpm --filter @relay-ai/server exec tsx src/cli/index.ts openapi
	pnpm --filter @relay-ai/console gen:api
	pnpm --filter @relay-ai/sdk gen:api
...
coverage: ## unit coverage with thresholds (business logic)
	pnpm --filter @relay-ai/server coverage
...
e2e: ## Playwright console E2E (start the stack first: make dev)   [sprint Day 13]
	pnpm --filter @relay-ai/console exec playwright install --with-deps chromium
	pnpm --filter @relay-ai/console e2e

sdk-e2e: ## @relay-ai/sdk end-to-end against the running gateway (make dev + seed-demo first)
...
audit-verify: ## re-walk every org's audit hash chain, fail on a break  [sprint Day 12/15]
	$(LOADENV) pnpm --filter @relay-ai/server exec tsx src/cli/index.ts audit verify
