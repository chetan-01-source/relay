...
import { type CanonicalRequest, type Target } from '../types/proxy.types.js';
...
describe('proxy.service', () => {
  it('complete() returns the upstream JSON', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn(async () => new Response(JSON.stringify({ id: 'x' }), { status: 200 })),
    );
    const out = await createProxyService().complete(req, target);
  ...
  it('complete() throws RelayError(upstream_error) passing the upstream status through', async () => {
    ...
    await expect(createProxyService().complete(req, target)).rejects.toMatchObject({
      code: 'upstream_error',
      status: 429,
    });
  });
  ...
  it('stream() yields normalized canonical deltas then stops on [DONE]', async () => {
    ...
    const texts: string[] = [];
    for await (const d of createProxyService().stream(req, target)) {
